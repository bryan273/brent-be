...
class InvertedIndex:
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            # load avg.dl
            self.postings_dict, self.terms, self.doc_length, self.avgdl = pickle.load(f)
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(self.metadata_file_path, 'wb') as f:
            # Simpan avg.dl di inverted index supaya tidak dihitung berkali-kali
            N = len(self.doc_length)
            avgdl = sum(self.doc_length.values()) / N
            pickle.dump([self.postings_dict, self.terms, self.doc_length, avgdl], f)
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        if term not in self.postings_dict:
            raise ValueError('term tidak dikenali')
        
        start_position, _, len_in_bytes_of_postings, len_in_bytes_of_tf = self.postings_dict[term]
        # pergi ke start position dari index term tsb
        self.index_file.seek(start_position)
        
        # read sebanyak byte yang disimpen and decode
        postings_list = self.postings_encoding.decode(
            self.index_file.read(len_in_bytes_of_postings)
        )
        tf_list = self.postings_encoding.decode_tf(
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        #    Ingat kembali bahwa self.postings_dict memetakan sebuah termID ke
        #    sebuah 4-tuple
        start_position_in_index_file = self.index_file.tell()  # the current position is at the end of the file
        ...
        self.postings_dict[term] = (start_position_in_index_file, number_of_postings_in_list,
                                    length_in_bytes_of_postings_list, length_in_bytes_of_tf_list)
        
        # Update terms dan doc_length
        self.terms.append(term)
        for doc_id, tf in zip(postings_list, tf_list):
            self.doc_length[doc_id] = self.doc_length.get(doc_id, 0) + tf  # Increment by term frequency
