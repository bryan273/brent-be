...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        """
        # Referensi buku teks index_compression halamaan 30
    ...
    def vb_encode(list_of_numbers):
        ...
        """
        # Cara konversi ke byte array numer per number
        # Ref: https://embeddedinventor.com/python-bytearray-everything-you-need-to-know/
        # encode number per number
        byte_stream = bytearray()
        for num in list_of_numbers:
            byte_stream.extend(VBEPostings.vb_encode_number(num))
    ...
    def encode(postings_list):
        ...
        bytes
            bytearray yang merepresentasikan urutan integer di postings_list
        """
        # debugging kalo posting list kosong
        if not postings_list:
            raise ValueError("Posting list tidak boleh kosong") 
        
        # simpan dalam gap, dan panggil encode ke bytearray dari vb_encode
        real_index = [postings_list[0]] 
        constructed_gap = [postings_list[i] - postings_list[i - 1] for i in range(1, len(postings_list))]
        gap_list = real_index + constructed_gap
